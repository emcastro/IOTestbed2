...
import iotestbed.boat.util.Criteria

object VesselDatabase_Thready extends App {
...

  {
    ...
    Utils.grouped(entries)(_ ("date")).foreach( // sequential foreach
      entryGroup => {
        println(entryGroup.mkString("---", "\n   ", ""))

        entryGroup.par.foreach(processEntry) // don't forget .par
      }
    )
  ...
  def dump() = {
    ...
    val completeHulls = hulls.par.map(hull => { // don't forget .par
  ...
  println(Schema.dump(results.toIndexedSeq).sorted.mkString("\n-----\n"))
